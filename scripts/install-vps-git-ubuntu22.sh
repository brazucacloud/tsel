...
# TSEL VPS Installation Script - Ubuntu 22.04+ Fixed
# Este script resolve completamente o problema do MongoDB em Ubuntu 22.04+
...
log "🚀 Iniciando instalação TSEL na VPS (Ubuntu 22.04+ Fixed)..."
...
# FIXED: MongoDB Installation for Ubuntu 22.04+
log "📦 Instalando MongoDB (Versão Corrigida para Ubuntu 22.04+)..."

# Remove any existing MongoDB installations and repositories
log "🧹 Limpando instalações anteriores do MongoDB..."
...
if [[ "$VERSION_ID" == "22.04" || "$VERSION_ID" == "23.04" || "$VERSION_ID" == "23.10" ]]; then
    warning "Detectado Ubuntu $VERSION_ID - Instalando MongoDB 7.0 (compatível)"
    ...
    success "MongoDB 7.0 instalado para Ubuntu $VERSION_ID"
else
    # For older Ubuntu versions, try MongoDB 6.0
    warning "Detectado Ubuntu $VERSION_ID - Tentando MongoDB 6.0"
    
    # Create keyring directory
    mkdir -p /etc/apt/keyrings
    
    # Add MongoDB 6.0 repository with proper keyring
    wget -qO - https://www.mongodb.org/static/pgp/server-6.0.asc | gpg --dearmor -o /etc/apt/keyrings/mongodb-server-6.0.gpg
    echo "deb [ arch=amd64,arm64 signed-by=/etc/apt/keyrings/mongodb-server-6.0.gpg ] https://repo.mongodb.org/apt/ubuntu focal/mongodb-org/6.0 multiverse" | tee /etc/apt/sources.list.d/mongodb-org-6.0.list
    
    # Update and install
    apt update
    apt install -y mongodb-org
    
    success "MongoDB 6.0 instalado para Ubuntu $VERSION_ID"
fi

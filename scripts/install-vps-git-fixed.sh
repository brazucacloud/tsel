...
# TSEL VPS Installation via Git Script (Fixed for Ubuntu 22.04+)
...
log "🚀 Iniciando instalação TSEL na VPS via Git (Versão Corrigida)..."
...
# Install MongoDB (Fixed for Ubuntu 22.04+)
...
if [[ "$VERSION_ID" == "22.04" || "$VERSION_ID" == "23.04" || "$VERSION_ID" == "23.10" ]]; then
    ...
    # Install MongoDB 6.0 for older versions
